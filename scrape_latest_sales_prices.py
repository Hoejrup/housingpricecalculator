...
def make_request_street(street: str, zip_code: str, property_type: PropertyType) -> bs4.BeautifulSoup:
...
def make_request_zipcode(zip_code: str, property_type: PropertyType) -> bs4.BeautifulSoup:
    """Make request to boliga.dk."""
    url = (f'https://www.boliga.dk/salg/'
           f'resultater?searchTab=1&propertyType={property_type.value}&zipcodeFrom={zip_code}&'
...
@main
def _(zip_code: str, streets_txt: str = None, property_type: PropertyType = PropertyType.house) -> None:
    """
    Scrape sales prices from boliga.dk

    Args:
        streets_txt: path to .txt file with street names (Optional)
    ...
    if streets_txt:
        ...
        for street in streets:
            soup = make_request_street(street, zip_code, property_type)
            try:
        ...
        soup = make_request_zipcode(zip_code, property_type)
        try:
            new_rows = scrape_prices(soup)
            rows.extend(new_rows)
        except NoSoldListError:
